...
public class Line extends Figure {
    ...
    public static void main(String[] args) {
        ...
        line.zoom(120);
        System.out.println("将line指定端点" + line.point.toString() + "缩放120% line:" + line);
        System.out.println("line2:" + line2);
        line2.revolve(10);
        System.out.println("将line2绕点" + line.point.toString() + "顺时针旋转10度    line2:" + line);
    ...
    public boolean contains(Point p) {
        ...
        if (p.x == point.x && p.y == point.y) return true;//p与point重合
            ...
            //运用向量等数学知识求解
            return x == 0.0 || (x != 0.0 && (p.y - point.y) / (p.x - point.x) == y / x);
        } else return false;
    }
    ...
    public double distance(Point p) {
        ...
        if (this.contains(p)) return 0.0;//点在直线上
            ...
            if (a * a + b * b - c * c <= 0 || b * b + c * c - a * a <= 0)
                return (a < c ? a : c);//若点与两端点的连线与直线成钝角，则点到该直线的距离是点到端点的最短距离

...
public class Ellipse extends ClosedFigure{
	...
	public void revolve(int angle){
		...
		new EllipseFrame(this.toString()+"旋转"+angle+"度",point,p,length,width,k);
	}
	...
	public boolean contains(Point p){
		...
		if(length>width){
			a=length/2;
			b=width/2;
			if((p.x-point.x)*(p.x-point.x)/(a*a)+(p.y-point.y)*(p.y-point.y)/(b*b)<=1) return true;
			else return false;
		}
		else{
			a=width/2;
			b=length/2;
			if((p.x-point.x)*(p.x-point.x)/(b*b)+(p.y-point.y)*(p.y-point.y)/(a*a)<=1) return true;
			else return false;
		}
	}
...
class EllipseFrame extends JFrame{
	...
	private double length,width,angle;

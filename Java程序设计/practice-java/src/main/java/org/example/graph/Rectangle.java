...
public class Rectangle extends ClosedFigure {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("将rectangle绕点" + new Point(rectangle.point.x + (rectangle.point2.x - rectangle.point.x) / 2,
                rectangle.point.y + (rectangle.point2.y - rectangle.point.y) / 2) + "顺时针旋转60度");
        System.out.print(" rectangle:");
        rectangle.print();
        rectangle.zoom(130);
        rectangle.draw();
    ...
    public boolean contains(Point p) {
        //判断点是否在长方形内
        return new Triangle(point, point2, point3).contains(p) || new Triangle(point, point2, point4).contains(p);
    }
}
